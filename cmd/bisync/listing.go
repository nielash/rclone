...
func ConvertPrecision(Modtime time.Time, DestPrecision time.Duration) time.Time {
	if Modtime.After(Modtime.Round(DestPrecision)) {
		return Modtime.Round(DestPrecision)
	}
	return Modtime
}
...
func (b *bisyncRun) modifyListing(ctx context.Context, src fs.Fs, dst fs.Fs, srcListing string, dstListing string, results []Results, queues queues, is1to2 bool) (err error) {
	...
	for _, result := range results {
		...
		if result.IsSrc && result.Src != "" && (result.Winner.Err == nil || result.Flags == "d") {
			srcWinners.put(result.Name, result.Size, ConvertPrecision(result.Modtime, src.Precision()), result.Hash, "-", result.Flags)
		...
		if result.IsWinner && result.Winner.Side != "none" && (result.Winner.Err == nil || result.Flags == "d") {
			dstWinners.put(result.Name, result.Size, ConvertPrecision(result.Modtime, dst.Precision()), result.Hash, "-", result.Flags)
	...
	if renames.NotEmpty() && !b.opt.DryRun {
		...
		for _, file := range renamesList {
			...
			dstList.put(newName, new.size, ConvertPrecision(new.time, src.Precision()), new.hash, new.id, new.flags)
			srcList.remove(file)
			dstList.remove(file)
		}
	}
