			...
			if atexit.Signalled() {
				if b.opt.Resync {
					...
					if b.SyncCI != nil {
						...
						if !waitFor("Canceling Sync if not done in", 30*time.Second, func() bool { return b.CleanupCompleted }) {
...
func (b *bisyncRun) runLocked(octx context.Context) (err error) {
	...
	if !bilib.FileExists(b.listing1) || !bilib.FileExists(b.listing2) {
		if b.opt.Resilient && bilib.FileExists(b.listing1+"-old") && bilib.FileExists(b.listing2+"-old") {
			...
			fs.Logf(nil, Color(terminal.YellowFg, "Listings not found. Reverting to prior backup as --resilient is set. \n")+errTip)
