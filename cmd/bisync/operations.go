...
func (b *bisyncRun) runLocked(octx context.Context, listing1, listing2 string) (err error) {
	...
	// debugging
	_, err1 = b.makeListing(fctx, b.fs1, listing1+"-control")
	_, err2 = b.makeListing(fctx, b.fs2, listing2+"-control")
	if fs.GetConfig(context.Background()).LogLevel >= fs.LogLevelDebug {
		err3 := b.checkSync(listing1+"-control", listing1)
		err4 := b.checkSync(listing2+"-control", listing2)
		fs.Debugf(nil, "errors: %v %v %v %v", err1, err2, err3, err4)
		// do an actual diff, as checkSync only compares file names
		DiffFiles(listing1+"-control", listing1, "Path1 Control", "Path1 New", "")
		DiffFiles(listing2+"-control", listing2, "Path2 Control", "Path2 New", "")
		DiffFiles(listing1, listing2, "Path1 New", "Path2 New", "")
	}
...
func (b *bisyncRun) resync(octx, fctx context.Context, listing1, listing2 string) error {
	...
	controlTest := func(listing1, listing2, label string) {
		fs.Debugf(nil, label)
		fmt.Printf("%q\n", label)
		...
		if fs.GetConfig(context.Background()).LogLevel >= fs.LogLevelDebug {
			...
			fs.Debugf(nil, "errors: %v %v %v %v", err1, err2, err3, err4)
			// do an actual diff, as checkSync only compares file names
			DiffFiles(listing1+"-control", listing1, "Path1 Control", "Path1 New", "-y --suppress-common-lines")
			DiffFiles(listing2+"-control", listing2, "Path2 Control", "Path2 New", "-y --suppress-common-lines")
			DiffFiles(listing1, listing2, "Path1 New", "Path2 New", "-y --suppress-common-lines")
		}
	}
	...
	controlTest(listing1, listing2, "resync 2to1")
	...
	controlTest(listing1, listing2, "resync 1to2")
	...
	controlTest(listing1, listing2, "resync 2to1 (dirs)")
...
// DiffStrings diffs two strings for testing
func DiffStrings(s1, s2, label1, label2 string) {
	fmt.Printf("Diff of %q and %q\n", label1, label2)
	cmd := exec.Command("bash", "-c", fmt.Sprintf(`diff <(echo "%s")  <(echo "%s")`, s1, s2))
	out, _ := cmd.Output()
	_, _ = os.Stdout.Write(out)
}

// DiffFiles diffs two files for testing
// "-y" for side-by-side
func DiffFiles(s1, s2, label1, label2, flags string) {
	fmt.Printf("\nDiff of %q and %q\n", label1, label2)
	cmd := exec.Command("bash", "-c", fmt.Sprintf(`diff %s %s %s`, s1, s2, flags))
	out, _ := cmd.Output()
	_, _ = os.Stdout.Write(out)
	fmt.Printf("\n\n")
}
