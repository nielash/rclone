	...
	if s.deleteMode == fs.DeleteModeOff {
		if s.usingLogger {
			...
			case fs.Directory:
				...
				s.logger(s.ctx, operations.MissingOnSrc, nil, operations.DirToObjInfo(dst, s.fdst), fs.ErrorIsDir)
				return true
			}
		}
		return false
...
func (s *syncCopyMove) SrcOnly(src fs.DirEntry) (recurse bool) {
	...
	case fs.Directory:
		...
		s.logger(s.ctx, operations.MissingOnDst, operations.DirToObjInfo(src, s.fsrc), nil, fs.ErrorIsDir)
		s.srcEmptyDirsMu.Unlock()
		return true
	default:
		panic("Bad object in DirEntries")
...
func (s *syncCopyMove) Match(ctx context.Context, dst, src fs.DirEntry) (recurse bool) {
	...
	case fs.Directory:
		...
		if ok {
			s.logger(s.ctx, operations.Match, operations.DirToObjInfo(src, s.fsrc), operations.DirToObjInfo(dst, s.fdst), fs.ErrorIsDir)
