...
func predictDstFromLogger(ctx context.Context) context.Context {
	...
	opt.LoggerFn = func(ctx context.Context, sigil operations.Sigil, src fs.ObjectInfo, dst fs.ObjectInfo, err error) {
		lock.Lock()
		defer lock.Unlock()
		...
		if winner.Obj != nil {
			file := winner.Obj
			hash, _ := file.Hash(ctx, file.Fs().Hashes().GetOne())
			errMsg := ""
			if winner.Err != nil {
				errMsg = ";" + winner.Err.Error()
			}
			operations.SyncFprintf(opt.JSON, "%s;%s;%v;%s%s\n", file.ModTime(ctx).Local().Format("2006-01-02 15:04:05.000000000"), hash, file.Size(), file.Remote(), errMsg)
		}
	}
...
func testLoggerVsLsf(ctx context.Context, Fremote fs.Fs, logger *bytes.Buffer, t *testing.T) {
	...
	err := LoggerMatchesLsf(logger, lsf)
	require.NoError(t, err)
}
